<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row gap-2 items-center">
            ...
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Test '. $test->name.' - przypisanie uczniów') }}
            </h2>
        </div>
    </x-slot>
    ...
    <script type="application/json" id="team_data">
        {!! json_encode([
            'id' => $test->id,
            'students' => $testStudentIds,
        ]) !!}
    </script>

    <script>
        const routes = {
            update: "{{ route('test.updateAssignStudent', ['test' => $test->id])}}",
            students: "{{ route('students.simplified', ['testId' => $test->id]) }}",
        }
